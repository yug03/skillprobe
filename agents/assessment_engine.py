...
class SkillState:
    ...
    def __init__(self, skill: str, claimed: float, required: float, importance: str):
        self.skill = skill
    ...
    def record(self, question: str, answer: str, difficulty: int, evaluation: dict):
...
class AssessmentEngine:
    ...
    def current_skill(self) -> str | None:
        while self.index < len(self.queue):
            ...
            skill_state = self.states.get(skill)
            if skill_state and skill_state.is_done:
                self.index += 1
                continue
            return skill
        self.done = True
    ...
    def generate_question(self) -> dict | None:
        skill_state = self.current_state()
        if skill_state is None:
            return None

        difficulty = skill_state.difficulty
        skill = skill_state.skill
        prev_qs = [h["question"] for h in skill_state.history]
        prev_block = ""
        if prev_qs:
            prev_block = "\n\nDo NOT repeat or rephrase these already-asked questions:\n" + "\n".join(f"- {q}" for q in prev_qs)

        difficulty_desc = getattr(config, "DIFFICULTY_DESCRIPTIONS", {}).get(difficulty, "")
...
({difficulty_desc})
Candidate claimed proficiency: {skill_state.claimed:.0%}
{prev_block}
...
"""
        ...
        return {
            ...
            "question_number": len(skill_state.history) + 1,
            "skill_number": self.index + 1,
            "total_skills": len(self.queue),
            "expected_depth": result.get("expected_key_points", []),
        }

    def submit_answer(self, question: str, answer: str, difficulty: int) -> dict:
        """
        ...
        skill_state.record(question, answer, difficulty, evaluation)

        skill_done = skill_state.is_done
        if skill_done:
            skill_state.observation = self._observe(skill_state)
            self.advance()
        ...
        return {
            ...
            "skill_summary": skill_state.to_dict() if skill_done else None,
            "next_skill": self.current_skill(),
        }

    def _observe(self, skill_state: SkillState) -> str:
        """Generate lightweight observation without extra LLM calls."""
        scores = [h["score"] for h in skill_state.history]
        avg = sum(scores) / len(scores) if scores else 0
        max_d = skill_state.max_diff
    ...
    def get_results(self) -> list[dict]:
        ...
        for skill in self.queue:
            skill_state = self.states.get(skill)
            if skill_state:
                if not skill_state.is_done:
                    skill_state._finalise()
                results.append(skill_state.to_dict())
        return results
